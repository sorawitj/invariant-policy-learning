...
from sim_utils.policy import RandomPolicy, Policy, LinearPolicy
import pandas as pd
import numpy as np
...
def verify_policies(s_size,
    ...
    trainer = PowerOpTrainer(100, 256, rate, model)
    for subset in target_sets.items():
        subset_name, subset_idx = subset
        linear_policy = LinearPolicy(subset_idx, n_actions, name='Linear_PG')
        trainer.train(X, A, Y, E, P, linear_policy, subset)
        new_P = linear_policy.get_prob(X, A)
        new_W = new_P.detach().numpy() / P
        target_name, p_val = invariance_test_resample(model, subset,
                                                      X, Y, E, new_W, rate)
